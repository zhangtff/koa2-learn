...
function getMongoUri() {
    ...
    if (mongodbConfig.get('user')) {
        mongoUri += `${mongodbConfig.get('user')}:${mongodbConfig.get('pass')}@${mongodbConfig.get('host')}:${mongodbConfig.get('port')}/${dbName}?authSource=admin`;
    } else {
        mongoUri += `${mongodbConfig.get('host')}:${mongodbConfig.get('port')}/${dbName}`;
    }
    // mongoUri += `/${dbName}`;
    return mongoUri;
}
